...
def create_gei(silhouette_folder):
    ...
    # Read all images and check their shape
    for frame_name in sorted(os.listdir(silhouette_folder)):  # Ensure order consistency
    ...
    target_size = (128, 128)
    silhouette_images_resized = [cv2.resize(img, target_size) for img in silhouette_images]

    # Convert list to NumPy array
    silhouette_stack = np.array(silhouette_images_resized, dtype=np.float32)

    # Compute GEI by averaging pixel values
    gei = np.mean(silhouette_stack, axis=0)

    # Normalize to 0-255 for saving
    gei = cv2.normalize(gei, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

    print(f"Created GEI with {len(silhouette_images)} valid silhouettes.")
